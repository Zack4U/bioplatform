...
public class GetSpeciesByIdQueryHandler : IRequestHandler<GetSpeciesByIdQuery, SpeciesDetailDTO>
{
    ...
    public async Task<SpeciesDetailDTO> Handle(GetSpeciesByIdQuery request, CancellationToken cancellationToken)
    {
        var species = await _speciesRepository.GetByIdWithDistributionsAsync(request.Id, cancellationToken)
    ...
    /// Admin, Researcher, EnvironmentalAuthority.
    /// </summary>
