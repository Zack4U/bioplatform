...
class TaxonomyInfo(BaseModel):
    ...
    kingdom: str = Field(
        default="", description="Biological kingdom (e.g., Animalia, Plantae)"
    )
    ...
    iucn_status: str = Field(
        default="", description="IUCN Red List status (LC, VU, EN, CR, etc.)"
    )
...
class GeoDistribution(BaseModel):
    ...
    latitude: Optional[float] = Field(
        None, description="Latitude (null if species is sensitive)"
    )
    longitude: Optional[float] = Field(
        None, description="Longitude (null if species is sensitive)"
    )
    altitude: Optional[float] = Field(None, description="Altitude in meters")
    observation_date: Optional[str] = Field(
        None, description="Date of observation (ISO 8601)"
    )
...
class SpeciesDbInfo(BaseModel):
    ...
    species_id: Optional[str] = Field(
        default=None, description="UUID of the species record"
    )
    common_name: Optional[str] = Field(
        default=None, description="Common/vernacular name"
    )
    description: Optional[str] = Field(
        default=None, description="Morphological description"
    )
    ecological_info: Optional[str] = Field(
        default=None, description="Habitat and ecology"
    )
    traditional_uses: Optional[str] = Field(
        default=None, description="Ethnobotanical / traditional uses"
    )
    economic_potential: Optional[str] = Field(
        default=None, description="Sustainable economic potential"
    )
    conservation_status: Optional[str] = Field(
        default=None, description="IUCN / Colombian Red Lists status"
    )
    is_sensitive: Optional[bool] = Field(
        default=None, description="Sensitive species flag (location masked)"
    )
...
class SpeciesPrediction(BaseModel):
    ...
    species: str = Field(
        ..., description="Predicted species name (binomial nomenclature)"
    )
    ...
    taxonomy: TaxonomyInfo = Field(
        default_factory=TaxonomyInfo, description="Full taxonomy"
    )
...
class ClassificationResponse(BaseModel):
    ...
    model: str = Field(
        ..., description="Model architecture used (e.g., efficientnet_b0)"
    )
    num_classes: int = Field(
        ..., description="Total number of classes the model can predict"
    )
...
class ModelInfoResponse(BaseModel):
    ...
    class_names: list[str] = Field(
        default=[], description="List of all species class names"
    )
