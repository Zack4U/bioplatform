...
def get_embeddings_batch(texts: list[str], api_key: str, retries: int = 5) -> list[list[float]]:
    ...
    for attempt in range(retries):
        try:
            ...
            if response.status_code == 429:
                wait_time = 90 
                print(f"[REINTENTO] Cuota excedida. Esperando {wait_time}s antes del intento {attempt + 1}...")
...
def main():
    ...
    if TRADITIONAL_USES_FILE.exists():
        ...
        for i, item in enumerate(uses_data):
            ...
            for j, use in enumerate(item.get("traditional_uses", [])):
                text_content = f"Especie: {species}\n" \
    ...
    for i in range(0, total_docs, batch_size):
        ...
        if not batch_ids:
            print(f"Lote {i//batch_size + 1} ya está completo en ChromaDB. Saltando...")
            continue

        print(f"Generando embeddings e ingestado lote {i//batch_size + 1}/{(total_docs//batch_size)+1} ({len(batch_ids)} nuevos documentos)...")
        
        try:
            batch_embeddings = get_embeddings_batch(batch_docs, gemini_api_key)
            
            if not batch_embeddings:
                print(f"[WARN] No se obtuvieron embeddings para el lote {i//batch_size + 1}")
                continue
