...
def get_embeddings_batch(texts: list[str], api_key: str) -> list[list[float]]:
    ...
    requests_data = [{"model": "models/gemini-embedding-001", "content": {"parts": [{"text": t}]}} for t in texts]
...
def main():
    ...
    try:
        ...
        # Productos
        cursor.execute("SELECT Name, Description, Composition, BasePrice FROM Products WHERE IsActive = 1")
        products = cursor.fetchall()
        
        # Permisos ABS
        cursor.execute("SELECT ResolutionNumber, Status, LegalFramework FROM AbsPermits")
    ...
    collection = client.get_or_create_collection(name=os.getenv("CHROMA_COLLECTION_NAME", "bioplatform_species_dev"))
    ...
    for i in range(0, len(docs), batch_size):
        ...
        if embeddings:
            collection.upsert(ids=b_ids, documents=b_docs, metadatas=b_meta, embeddings=embeddings)
