    ...
    with open(output_sql, 'w', encoding='utf-8') as sql_file:
        ...
        for ds_dir in [test_dir, val_dir]:
            ...
            for species_folder in os.listdir(ds_dir):
                ...
                # Transform species_folder (e.g., Abracris_flavolineata) to scientific name (Abracris flavolineata)
                ...
                for image in images:
                    ...
                    sql_file.write(
                        "INSERT INTO species_images (id, species_id, image_url, is_primary, license_type, is_validated_by_expert, created_at) "
                        f"SELECT gen_random_uuid(), id, '{s3_url}', {'true' if is_thumbnail else 'false'}, 'Unknown', true, NOW() "
...
if __name__ == '__main__':
    ...
    parser.add_argument('--base_dir', default='src/Bio.Backend.AI/data/processed', help='Directorio base de dataset processed')
    parser.add_argument('--bucket', default='bioplatform-public', help='Nombre del bucket de S3 publico')
    parser.add_argument('--out', default='src/Bio.Backend.AI/data/species_catalog/insert_species_images.sql', help='Archivo SQL generado')
